...
def main(
    ...
    batch_size_val=256,
    critic_learning_rate_val=3e-4,
    actor_learning_rate_val=3e-4,
    alpha_learning_rate_val=3e-4,
    ...
    eval_interval_val=500,
    policy_save_interval_val=300,
    model_type="SacAgent"):
    ...
    print("+++++")
    print(train_dir) #os.path.join(tempdir, learner.TRAIN_DIR))
...
def run_episodes_and_create_video(saved_policy, tf_env, job_id=""):
    ...
    print("after eval actor")
    def get_eval_metrics():
        print("inside get_eval_metrics")
        eval_actor.run()
        results = {}
        for metric in eval_actor.metrics:
            results[metric.name] = metric.result()
