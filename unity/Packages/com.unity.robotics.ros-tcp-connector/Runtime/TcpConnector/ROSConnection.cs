...
public class ROSConnection : MonoBehaviour
{
    ...
    byte[] ReadMessageContents(NetworkStream networkStream, out string topicName)
    {
        ...
        // Read until the declared length is satisfied, NOT until DataAvailable
        // goes false. DataAvailable only reports what is buffered right now, so
        // upstream this loop exits the moment the sender pauses mid-message and
        // returns a half-filled buffer. Under ~64 KB that is invisible (the
        // whole message is normally buffered before the first Read), which is
        // why it went unnoticed; the fly-brain overlay's 359 KB geometry
        // payload hit it on every single send and arrived as truncated JSON.
        //
        // Read blocks until at least one byte is available and returns 0 only
        // once the peer has closed, so the guard below is the real terminator.
        // Each message arrives on its own connection (HandleConnectionAsync
        // reads exactly one), so bailing early here loses just this message
        ...
        while (bytesRemaining > 0)
        {
            ...
            if (bytesRead <= 0)
            {
                Debug.LogWarning($"Message on '{topicName}' ended after "
                                 + $"{totalBytesRead} of {full_message_size} bytes.");
                break;
            }
