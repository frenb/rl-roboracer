...
public class MoveService : MonoBehaviour, IRosComponent
{
    ...
    {
        niryoOne = SimController.instance.car;

        jointArticulationBodies = new ArticulationBody[numRobotJoints];
        string shoulder_link = "world/base_link/shoulder_link";
        jointArticulationBodies[0] = new ArticulationBody();//niryoOne.transform.Find(shoulder_link).GetComponent<ArticulationBody>();

        string arm_link = shoulder_link + "/arm_link";
        jointArticulationBodies[1] = new ArticulationBody();//niryoOne.transform.Find(arm_link).GetComponent<ArticulationBody>();

        string elbow_link = arm_link + "/elbow_link";
        jointArticulationBodies[2] = new ArticulationBody();//niryoOne.transform.Find(elbow_link).GetComponent<ArticulationBody>();

        string forearm_link = elbow_link + "/forearm_link";
        jointArticulationBodies[3] = new ArticulationBody();//niryoOne.transform.Find(forearm_link).GetComponent<ArticulationBody>();

        string wrist_link = forearm_link + "/wrist_link";
        jointArticulationBodies[4] = new ArticulationBody();//niryoOne.transform.Find(wrist_link).GetComponent<ArticulationBody>();

        string hand_link = wrist_link + "/hand_link";
        jointArticulationBodies[5] = new ArticulationBody();//niryoOne.transform.Find(hand_link).GetComponent<ArticulationBody>();
        ...
        gripperBase = new GameObject("gripper").transform;//niryoOne.transform.Find(gripper_base);
        leftGripperGameObject = new GameObject("leftGripper").transform;//niryoOne.transform.Find(left_gripper);
        rightGripperGameObject = new GameObject("rightGripper").transform;//niryoOne.transform.Find(right_gripper);

        rightGripper = new ArticulationBody();//rightGripperGameObject.GetComponent<ArticulationBody>();
        leftGripper = new ArticulationBody();//leftGripperGameObject.GetComponent<ArticulationBody>();
    }
